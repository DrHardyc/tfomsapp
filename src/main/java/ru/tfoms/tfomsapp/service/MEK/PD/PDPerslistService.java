...
public class PDPerslistService {
    ...
    public PDPerslist loadPDPerslist(InputStream inputStream, String fileStructureName){
        ...
        List<Pers> pers = new ArrayList<>();
        for (Element child : childs){
            switch (child.getLocalName()){
                case "ZGLV" -> essPdPerslist.setZglv(pdZglvService.loadPdZglv(child));
                case "ZAP" -> pers.add(pdPersService.loadPers(child));
            }
        }
